...
class MQTTClient:
    def __init__(self):
        ...
        # Message deduplication - store hashes of recent messages
        self.processed_messages = deque(maxlen=100)  # Keep last 100 message hashes
    ...
    def _on_connect(self, client, userdata, flags, rc):
        ...
        if rc == 0:
            ...
            # Subscribe to unified sensor and device topics with QoS 0 to avoid duplicates
            topics = [
                "greenhouse/sensors/",  # For sensor data
                "greenhouse/devices/"   # For device status data
            ]
            for topic in topics:
                self.client.subscribe(topic, qos=0)  # Changed to QoS 0
    ...
    def _on_message(self, client, userdata, message):
        ...
        try:
            ...
            message_data = f"{topic}:{json.dumps(payload, sort_keys=True)}"
            message_hash = hashlib.md5(message_data.encode()).hexdigest()
            
            # Check if we've already processed this message
            if message_hash in self.processed_messages:
                logger.debug(f"Duplicate message detected, skipping: {message_hash[:8]}...")
                return
            ...
            if topic == "greenhouse/sensors/":
                ...
                if success:
                    mqtt_monitor.on_message_received('sensor_data')
                else:
                    mqtt_monitor.on_error('invalid_sensor_data')
                    
            elif topic == "greenhouse/devices/":
                # Process device status batch
                success = self._process_devices_data(payload)
                if success:
                    mqtt_monitor.on_message_received('device_status')
                else:
                    mqtt_monitor.on_error('invalid_device_data')
            else:
            ...
            mqtt_monitor.on_error('message_processing_error')
    ...
    def publish_device_command(self, device_type: str, command: Dict[str, Any]) -> bool:
        ...
        topic = Config.MQTT_TOPICS.get(device_type, f"greenhouse/control/{device_type}")
